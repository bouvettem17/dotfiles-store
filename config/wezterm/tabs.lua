...
function M.title(tab, max_width)
	...
	local folderName = title:match("([^/\\]+)$") or title
	local process = title:match("^(%S+)%s*%-?%s*%s*(.*)$")

	if M.icons[process] then
		folderName = M.icons[process] .. " " .. (folderName or "")
	end
	...
	folderName = wezterm.truncate_right(folderName, max_width - 3)
	return " " .. folderName .. " "
end
